...
- **Import (`server -i file.json` or `client workflow import file.json`)**:
  ...
  - The `client workflow import` variant writes directly to the database without starting the server or client runner.
- **Clone (`server -c file.json` or `client workflow clone file.json`)**:
  ...
  - The `client workflow clone` variant writes directly to the database without starting the server or client runner.
