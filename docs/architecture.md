...
3. **Distributed dispatch**: `IngestResult` evaluates the node's conditions into a `RoutingDecision` (per-entry booleans, derived `nexts`, first error), attaches it to the `ResultData` (`condition_results`/`nexts`/`condition_error`), dual-writes the result (DB `workflow_results` table + `./results/<result_id>.json` mirror), then persists one pending assignment per downstream `next` (idempotent per workflow/execution/node) and marks the reporting node done. `POST /api/results` returns only caller-owned nexts; foreign nexts are picked up via the `pending` array in `GET /api/clients/{id}/assignments` with tenant isolation and workflow-default resolution.
4. **Credentials**: nodes carry `credential_ref` (id or name); secrets live only in the encrypted `Credential` store (AES-256-GCM, `CREDENTIALS_KEY`). The server never ships secrets inside assignments — the executing client fetches just-in-time via `GET /api/credentials/{ref}/fetch` (client-token auth, tenant-isolated, `user > group > tenant` resolution, 60s TTL, audited as `credential.fetch`).
5. **RBAC + web auth**: `AppUser` rows (bcrypt passwords, `admin > operator > viewer`, optional `groups`) and `AuthSession` tokens (24h TTL) gate every management endpoint; the website uses a session cookie with login redirect, plus `/profile`, `/credentials`, `/clients`, `/audit`, and `/users` pages (navbar hidden until login). Enrollment, client revoke, credential mutates, and workflow import/clone/enable/delete require operator+ (cross-tenant requires admin); user management and cross-user password resets are admin-only, while every user changes their own password via `/profile` or `POST /api/auth/password`. Every auth, user, token, workflow, and credential op is audit-logged with the session user as actor.
