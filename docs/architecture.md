...
5. **RBAC + web auth**: `AppUser` rows (bcrypt passwords, `admin > operator > viewer`, optional `groups`) and `AuthSession` tokens (24h TTL) gate every management endpoint; the website uses a session cookie with login redirect, plus `/audit` and `/users` pages. Enrollment, client revoke, credential mutates, and workflow import/clone/enable require operator+ (cross-tenant requires admin); user management is admin-only. Every auth, user, token, workflow, and credential op is audit-logged with the session user as actor.
